...
$\scriptsize\int_0^t dW_s^2 = \lim_{n \rightarrow \infty} \sum_{k=1}^n \Delta W_{t_k}^2$ 이므로 <span style="color:gray">(where $$\Delta W_{t_k} = W_{t_k}-W_{t_{k-1}}, \Delta t = \frac{t}{n}, \quad 0<t_1<t_2<\cdots<t$$)</span>

mean-square convergence 식에서 $$X_n=\sum_{k=1}^n \Delta W_{t_k}^2, \ X=t$$라고 보면

$$\scriptsize\lim_{n \rightarrow \infty} \mathbb{E}\Bigr[\Bigl(\sum_{k=1}^n \Delta W_{t_k}^2 - t \Bigl)^2\Bigr] = 0$$

이제 식을 전개해보면

$$\scriptsize\lim_{n \rightarrow \infty} \mathbb{E}\Bigr[\Bigl(\sum_{k=1}^n \Delta W_{t_k}^2 \Bigl)^2 -2t (\sum_{k=1}^n \Delta W_{t_k}^2 \Bigl) + t^2 \Bigr] = 0$$

$$\scriptsize\lim_{n \rightarrow \infty} \mathbb{E}\Bigr[\Bigl(\sum_{k=1}^n \Delta W_{t_k}^2 \Bigl)^2 \Bigr] -2t \cdot \mathbb{E} \Bigr[(\sum_{k=1}^n \Delta W_{t_k}^2 \Bigl)\Bigr] + t^2 = 0$$
...
이 내용은 [이 포스트](https://rouxist.github.io/posts/math_etc_expectation-rv-to-power/)에 정리되어있음
...
$$\scriptsize\lim_{n \rightarrow \infty} \mathbb{E}\Bigr[\Bigl(\sum_{k=1}^n \Delta W_{t_k} \Delta t_k \Bigl)^2\Bigr] = 0$$
...
[Brownian motion의 정의](https://rouxist.github.io/posts/math_sc_part-2/#24-brownian-motion)에 의해 $\Delta W_{t_j}$ 와 $\Delta W_{t_j} $는 독립이므로, $\mathbb{E}\Bigr[\Delta W_{t_i} \Delta W_{t_j} \Bigr]=\mathbb{E}\Bigr[\Delta W_{t_i} \Bigr] \mathbb{E}\Bigr[ \Delta W_{t_j} \Bigr]$ 이고 각각의 값이 0이므로 두 번째 항의 값은 0이 된다.
